...
// initialise 31 days of the month for default month January
function initDay() {
...
// add or remove days for the selected month and year
//this is called when either the month or year select options are changed
function fillDay() {
	var dayobj=document.getElementById("dayob");
	var monthobj=document.getElementById("monthob");
	var yearobj=document.getElementById("yearob");
	var currentyear=parseInt(yearobj.options
		[yearobj.selectedIndex].value);
	var currentmonth=parseInt(monthobj.options
		[monthobj.selectedIndex].value);
...
function checkUser(username,email,callback) {
	...
	var XHR=createXHR();
	XHR.open("POST","/~ar02lw/hnd/registerdemo/php/checkUser.php",true);
	XHR.onreadystatechange = (function() {
		if(XHR.readyState==4) {
			if(XHR.status==200) {
				if(XHR.response=="Could not connect to database: 1045")
				{
					// Database connectivity issues
					usernameFb.innerHTML="Could not process request, try again later";
					useravailable=false;
				} else {
...
function checkForm(e) {
	...
	with(theform) {
		...
		if(checkEmpty(firstname.value))
			{ firstnameFb.innerHTML="Please add your forename/s"; 
				iscomplete=false;}
		else
			{ firstnameFb.innerHTML=""; }
		if(checkEmpty(surname.value)) 
			{ surnameFb.innerHTML="Please add your surname";
				iscomplete=false;}
		else
			{ surnameFb.innerHTML=""; }
		if(checkEmpty(username.value)) 
			{ usernameFb.innerHTML="Please add your desired username";
				iscomplete=false; }
		else
			{ usernameFb.innerHTML=""; }
		if(checkEmpty(userpass.value)) 
			{ userpassFb.innerHTML="Please enter a password";
				iscomplete=false; testuser=false;}
		else
			{ userpassFb.innerHTML=""; }
		if(checkEmpty(secondpass.value)) 
			{ secondpassFb.innerHTML="Please re-enter your password";
				iscomplete=false;}
		else
			{ secondpassFb.innerHTML=""; }
		if(checkPass(userpass.value,secondpass.value))
			{secondpassFb.innerHTML="Passwords do not match";
				iscomplete=false;}
		if(checkTerms(tnc))
			{tncFb.innerHTML=
				"You must agree to the website terms and conditions"; 
				iscomplete=false;}
		else
			{tncFb.innerHTML="";}
		if(checkAge(yearob.value,monthob.value,dayob.value))
			{ ageFb.innerHTML="Sorry, you must be 16 to register.";
				iscomplete=false;}
		else
			{ ageFb.innerHTML=""; }
		if(checkEmail(emailadd.value))
			{emailFb.innerHTML="Please enter a valid email address";
				iscomplete=false; testuser=false;}
		else
			{emailFb.innerHTML="";}
		//check user function with callback from XHR to check if user or 
		//email exists.
		if(testuser && typeof JSON==="object") {
			checkUser(username.value,emailadd.value,function(useravailable){
				submitb.disabled=false;
				if (useravailable && iscomplete) {
					theform.submit();
				}
			});
		} else if (iscomplete) {
...
if(window.addEventListener) {
		document.getElementById("monthob").addEventListener
			("change",fillDay,false);
		document.getElementById("yearob").addEventListener
			("change",fillDay,false);
		document.getElementById("userpass").addEventListener
			("keyup", testStrength, false);
		document.getElementById("registeruser").addEventListener
			("submit",checkForm,false);
	} else if(window.attachEvent) {
		document.getElementById("monthob").attachEvent
			("onchange",fillDay);
		document.getElementById("yearob").attachEvent
			("onchange",fillDay);
		document.getElementById("userpass").attachEvent
			("onkeyup", testStrength);
		document.getElementById("registeruser").attachEvent
			("onsubmit",checkForm);
	}
}
